...
def save_password(website, mail, password):
    ...
    if is_ok:
        password_csv_exists = os.path.exists("password.csv")
        password_data = {
            "website": [f"{website}"],
            "username": [f"{mail}"],
            "password": [f"{password}"]
        }
        pd.DataFrame(password_data).to_csv("password.csv", mode="a", header=not password_csv_exists)
...
website_input.focus()
website_input.grid(row=1, column=1, columnspan=2, sticky="ew", padx=20, pady=2)
